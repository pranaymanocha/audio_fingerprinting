...
if [ $(($parser)) -lt 160 ]; then

    OUTPUT2="$(qsub -q $qsz1 -N last_part -e $errPt -o $outPt -W depend=afterany:"$(join_by : "${array[@]}")" -v stnno=$stn,styp=$sttyp,maxp=$max,star=$parse,logf=$logfl submit_script2.sh)"
else
OUTPUT2="$(qsub -q $qsz2 -N last_part -e $errPt -o $outPt -W depend=afterany:"$(join_by : "${array[@]}")" -v stnno=$stn,styp=$sttyp,maxp=$max,star=$parse,logf=$logfl submit_script2.sh)"
fi
