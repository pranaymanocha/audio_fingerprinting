...
do
    ...
    do
        ...
        if [ $main_main -gt 1 ]; then
                   OUTPUT="$(qsub -q $qsz1 -N $evJbnm -W depend=afterany:"$(join_by : "${array4[@]}")" -e $errPt -o $outPt -v stnno=$stn,styp=$sttyp,maxp=$max,star=$COUNTER,logf=$logfl submit_script1.sh)"               
            else
