...
declare -a stno=(50 100 200 300)
declare -a strtyp=(5 10)
declare -a maxpeak=(1 10 50 100)
declare -a array4=()
main_main=0
...
#OUTPUT="$(qsub -q $qsz2 -N $evJbnm -W depend=afterany:"$(join_by : "${array5[@]}")" -e $errPt -o $outPt -v stnno=$stn,styp=$sttyp,maxp=$max,star=$COUNTER,logf=$logfl submit_script1.sh)"
#main_main=$((main_main+1))
#fi
...
#fi
 ...
 #OUTPUT="$(qsub -q $qsz1 -N $evJbnm -e $errPt -o $outPt -v stnno=$stn,styp=$sttyp,maxp=$max,star=$COUNTER,logf=$logfl submit_script1.sh)"
  ...
  #       array+=("$(echo "$OUTPUT" | cut -c1-17)")
         #echo $array 
         let COUNTER=COUNTER+800
        let count23=count23+1
 #      done                            
       echo "Hello"   
#array=(a b c d e)
#cnt=${#array[@]}
#for ((i=0;i<cnt;i++)); do3    array[i]="${array[i]}.lti.cs.cmu.edu"
#    echo "${array[i]}"
#done      
echo "Hello1"
...
#done
 ...
 #       logfl=$log/"log."$evJbnm
  ...
  #     OUTPUT1="$(qsub -q $qsz1 -N neglet -e $errPt -o $outPt -W depend=afterany:"$(join_by : "${array[@]}")" -v logf=$logfl submit_script3.sh) "
...
if [ $(($parser)) -lt 150 ]; then

    OUTPUT2="$(qsub -q $qsz1 -N last_part -e $errPt -o $outPt -v stnno=$stn,styp=$sttyp,maxp=$max,logf=$logfl submit_script2.sh)"
else
OUTPUT2="$(qsub -q $qsz2 -N last_part -e $errPt -o $outPt -v stnno=$stn,styp=$sttyp,maxp=$max,logf=$logfl submit_script2.sh)"
fi
...
echo "Hello3"
    ...
    #  OUTPUT3="$(qsub -q $qsz1 -N last_last -e $errPt -o $outPt -W depend=afterany:"$(join_by : "${array2[@]}")" -v logf=$logfl submit_script4.sh )"
