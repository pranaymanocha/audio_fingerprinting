...
if [ $(($parser)) -lt 150 ]; then
    ...
    OUTPUT2="$(qsub -q $qsz1 -N last_part -e $errPt -o $outPt -v stnno=$stn,styp=$sttyp,maxp=$max,star=$parse,logf=$logfl submit_script2.sh)"
else
OUTPUT2="$(qsub -q $qsz2 -N last_part -e $errPt -o $outPt -v stnno=$stn,styp=$sttyp,maxp=$max,star=$parse,logf=$logfl submit_script2.sh)"
fi
